...
* Emacs Settings
+ TODO Try get save and restore window working
    #+begin_src emacs-lisp
      (use-package emacs
	:ensure nil
	:config
	...
	;; 			       (let ((current-frame-parameters (frame-parameters (selected-frame))))
	;; 				 (when (fboundp 'w32-send-sys-command)
	;; 				   ;; For Windows system
	;; 				   (w32-send-sys-command #xf020 current-frame-parameters))
	;; 				 ;; Save frame parameters
	;; 				 (set-frame-parameter nil 'fullscreen (frame-parameter nil 'fullscreen))
	;; 				 (customize-save-variable 'initial-frame-alist (list (cons 'top (cdr (assoc 'top current-frame-parameters)))
	;; 										     (cons 'left (cdr (assoc 'left current-frame-parameters)))
	;; 										     (cons 'height (cdr (assoc 'height current-frame-parameters)))
	;; 										     (cons 'width (cdr (assoc 'width current-frame-parameters)))))))))
    #+end_src
...
* My Functions
** TODO Disabling this while evaluating Elpaca
    + Someone mentioned that stuff needs to be in ~use-package~ blocks for the first pass... yeah I don't really know what I am saying either.
    #+begin_src emacs-lisp :tangle no
